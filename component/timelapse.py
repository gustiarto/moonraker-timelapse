...
class Timelapse:
    ...
    async def webrequest_settings(self,
        ...
        if action == 'POST':
            ...
            settingsWithGcodechange = [
                ...
                'park_dynamic_enabled', 'park_dynamic_y_min', 'park_dynamic_y_max'
            ]
            modechanged = False
    ...
    async def setgcodevariables(self) -> None:
        gcommand = "_SET_TIMELAPSE_SETUP " \
            ...
            + f" PARK_DYNAMIC_ENABLE={self.config.get('park_dynamic_enabled', False)}" \
            + f" PARK_DYNAMIC_Y_MIN={self.config.get('park_dynamic_y_min', 30.0)}" \
            + f" PARK_DYNAMIC_Y_MAX={self.config.get('park_dynamic_y_max', 180.0)}"
